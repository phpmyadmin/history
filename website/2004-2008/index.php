...
<?php
    ...
    // quick downloads
    echo renderBoxHeader(350, 'QUICK DOWNLOADS', 'downloads.php', '_self');
    
    // Show latest stable3 release
    for ($stable3=0; $stable3<$releases; $stable3++) {
        if ($release[$stable3]['type'] == 'stable3') break;
    }
    echo renderQuickDownloads($stable3, 'Latest stable 3.x version');
	...
	echo renderQuickDownloads($stable2, 'Latest stable 2.x version');
    }

    // If there is newer RC than stable, show it here also
    for ($rc=0; $rc<$stable3; $rc++) {
    ...
    for ($alpha=0; $alpha<$stable3; $alpha++) {
    ...
    echo renderBoxFooter(350, 'downloads.php', '_self', '...more download options', '... downloads', TRUE);

    // project info
    echo renderBoxHeader(350, 'PROJECT INFO', '', '');
?>
...
<?php
    ...
    // LinuxTag 2005
    echo renderBoxHeader(350, 'LINUXTAG 2005', '', '');
?>
...
<?php
    ...
    // news (rss - feed)
    echo renderBoxHeader(350, 'NEWS', '', '');
    //include($import['rssfeed']);
    //$pma_rss_newsfull = new pmaRSSFeed('newsfull');
?>
            <table align="center" width="239" border="0" cellpadding="0" cellspacing="0">
<?php
/*
...
?>
                <tr>
                    <td colspan="3" height="25" class="content" align="left"><b>Please visit our <span class="gotopage"><a href="http://sourceforge.net/news/?group_id=<?php echo $sf_group_id; ?>">News-Archive</a></span>.</b></td>
                </tr>
            </table>
<?php
    //unset($max);
